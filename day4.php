...
$input = "[1518-04-25 00:24] falls asleep
...
foreach($record as $line) {
    ...
    if($char === '#') {
        ...
        $guard = (int) $second;
        if(!isset($guards_sleeping_minutes[$guard])) {
            $guards_sleeping_minutes[$guard] = array_fill_keys(range(0,59), 0);
        }
        continue;
    } 
    preg_match('/^\[\d+-(?<month>\d+)-(?<day>\d+) (?<hour>\d+):(?<minute>\d+)/', $line, $matches);
    ...
    if($char === 'a') { 
        ...
        if($asleep['day'] != $awake['day']) {
            // we can assume they started sleeping just the day before
            for($i = (int) $asleep['minute']; $i < 60; $i++) {
                $guards_sleeping_minutes[$guard][$i]++;
            }
            $asleep['minute'] = 0;
        }
        $awake['minute'] = (int) $awake['minute'];
        for($i = (int) $asleep['minute']; $i < $awake['minute']; $i++) {
            $guards_sleeping_minutes[$guard][$i]++;
        }
    }
}
...
$total_sleep = 0;
$selected_minute = -1;
$selected_most_minutes = -1;
$selected_guard = -1;
foreach($guards_sleeping_minutes as $guard => $minutes) {
    asort($minutes);
    end($minutes);
    $minute = key($minutes);
    $most_minute = current($minutes);

    if($most_minute > $selected_most_minutes) {
        $selected_guard = $guard;
        $selected_minute = $minute;
        $selected_most_minutes = $most_minute;
    }
}

print "guard $selected_guard has slept a total of $selected_most_minutes minutes with the most on minute $selected_minute giving us " . 
($selected_guard * $selected_minute);